...
import CartDrawer from '../Content/CartDrawer';
...
const handleRemove = (index) => {
  ...
  const navConfig = [
    {
      label: 'Home',
      items: ['Home Default', 'Home Modern', 'Home Creative', 'Home Local Store', 'Home Minimalist']
    },
    {
      label: 'Shop',
      items: [
    ...
    {
      label: 'Stores',
      items: ['Store List', 'Store Grid', 'Store Single']
    },
    {
      label: 'Mega menu',
      items: ['Dairy, Bread & Eggs', 'Breakfast & Instant Food', 'Cold Drinks & Juices']
    },
    {
      label: 'Pages',
      items: ['Blog', 'Blog Single', 'Blog Category', 'About us', '404 Error', 'Contact']
    }
  ];
  ...
  <>
      ...
      <div className="block md:hidden px-4 pt-3 py-2  ">
        <div className="flex items-center justify-between bg-white ">
          <div className="flex items-center gap-1 text-green-600 font-bold text-xl">
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24" fill="currentColor"><path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2zM7.2 14h9.9c.75 0 1.41-.41 1.75-1.03l3.24-6.16c.08-.14.11-.3.11-.46 0-.55-.45-1-1-1H5.21L4.27 2H1v2h2l3.6 7.59-1.35 2.44c-.18.31-.28.65-.28 1.01 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63z" /></svg>
            <span>FreshCart</span>
          </div>
          <div className="flex items-center gap-4 text-gray-600 text-xl">
            <div className="relative">
              <FaHeart />
              <span className="absolute -top-2 -right-2 bg-green-600 text-white text-xs px-1.5 rounded-full font-medium">4</span>
            </div>
             <button onClick={() => setModalOpen(true)} className="relative">
            <FaUser className="text-xl text-gray-600 hover:text-green-600" />
          </button>
            <div className="relative">
               <button onClick={() => setCartOpen(true)} className="relative">
          <FaShoppingBag className="text-xl text-gray-600 hover:text-green-600" />
          <span className="absolute -top-2 -right-2 bg-green-600 text-white text-xs px-1.5 rounded-full">
            5
          </span>
          </button>
            </div>
            <IoMdMenu onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-green-600 text-2xl" />
          </div>
        </div>
    ...
    <header className="w-full border-b border-gray-200 bg-white ">
      ...
      <div className="bg-gray-100 text-gray-500 text-sm font-medium ">
        <div className="container mx-auto flex justify-between items-center px-10 py-1">
...
<script xmlns=""/></svg>
        ...
        <div className="flex flex-1 max-w-xl mx-4 items-center">
          <div className="flex flex-1 border border-gray-300 rounded-md overflow-hidden">
            <input
              type="text"
          ...
          <button className="ml-2 flex items-center border border-gray-300 rounded-md px-4 py-2 text-sm text-gray-700">
      ...
      {/* Navigation + Dropdown */}
      <nav className="container mx-auto px-10 py-3 text-sm flex items-center gap-6 relative z-10">
        {/* All Departments Dropdown */}
        <div
          className="relative"
          onMouseEnter={() => setDropdown('departments')}
          onMouseLeave={() => setDropdown(null)}
        >
          <button className="flex items-center bg-green-600 text-white px-4 py-2 rounded-md font-medium gap-2">
            <MdApps className="text-lg" />
            All Departments
          </button>

          {dropdown === 'departments' && (
            <div className="absolute top-full left-0  w-64 bg-white border border-gray-200 shadow-lg rounded-md z-3 p-2">
              {[
                'Dairy, Bread & Eggs',
                'Snacks & Munchies',
                'Fruits & Vegetables',
                ...
                <Link
                  key={i}
                  href="#"
                  className="block px-4 py-2 text-gray-800 hover:bg-gray-100 transition duration-150 delay-100 rounded-md"
                >
                  {item}
                </Link>
              ))}
        ...
        {navConfig.map((config) => (
          <div
            key={config.label}
            className="relative "
            onMouseEnter={() => setDropdown(config.label)}
            onMouseLeave={() => setDropdown(null)}
          >
            <button className="flex items-center text-gray-800 hover:text-green-600 gap-1 cursor-pointer ">
              {config.label}
              <IoIosArrowDown className="text-xs" />
            </button>
            {dropdown === config.label && (
              <div className="absolute top-full  min-w-[180px] bg-white border border-gray-200  rounded-md shadow-lg py-2 cursor-pointer p-2 transition delay-150 duration-100">
                {config.items.map((item, i) => (
                  <Link
                    key={i}
                    href="#"
                    className="block px-4 py-2 text-gray-600 rounded-md hover:bg-gray-100 hover:text-gray-800 "
                  >
                    {item}
                  </Link>
                ))}
        ...
        <Link href="#" className="text-gray-800 hover:text-green-600">Docs</Link>
      </nav>
    </header>
      </div>
...
>
  <div className="flex justify-between items-center p-4 border-b border-gray-100">
    ...
    <button onClick={() => setIsMobileMenuOpen(false)} className="text-2xl text-gray-700">×</button>
  </div>
  ...
  <div className="p-4">
    ...
    <button className="w-full bg-green-600 text-white font-semibold py-2 rounded-md flex items-center justify-center gap-2 mb-4">
    ...
    {navConfig.map((nav, i) => (
      <div key={i} className="mb-3">
        <button
          ...
          className="w-full text-left flex justify-between items-center text-gray-800 py-2 border-b border-gray-100 font-medium"
        >
          {nav.label}
          <span className={`text-gray-500 transition-transform ${dropdown === i ? 'rotate-180' : ''}`}><IoIosArrowDown /></span>
        </button>
        {dropdown === i && (
          <div className="bg-gray-50 p-3 rounded-md mt-1">
            {nav.items.map((item, idx) => (
              <Link key={idx} href="#" className="block py-1 text-gray-600">
                {item}
              </Link>
            ))}
          </div>
    ...
    <Link href="/dashboard" className="block py-2 border-b border-gray-100 text-gray-800 font-medium">Dashboard</Link>
    <Link href="#" className="block py-2 text-gray-800 font-medium">Docs</Link>
  </div>
</div>
